...
figname <- paste0(a, b,'_latVsDepth')
...
par(mar = c(3,3,1,1))
plot(depthMean1[,2,1], depthMean1[,3,1], type = 'n',ylim = ylim, xlim = xlim, ylab = '', xlab = '', axes = F)
...
for(i in 1:3){
  lines(depthMean1[,2,i], depthMean1[,3,i], lty = lty[i], col = 'red' , lwd = c(2,2,2))
  points(depthMean1[1,2,i], depthMean1[1,3,i], col = 'red', cex = 3.)
  
  lines(depthMean2[,2,i], depthMean2[,3,i], lty = lty[i], col = 'blue', lwd = c(2,2,2))
  points(depthMean2[1,2,i], depthMean2[1,3,i], col = 'blue', cex = 3.)
}
