...
for(i in seq(1,26,5)){
  ...
  image.plot(lon, lat, mask, xlim = xlimmap, ylim = ylimmap)
  points(sub_day$Lon, sub_day$Lat)
...
pngfile <- paste0(out_path, 'Sechura_path_out.png')
...
write.table(x = df, file = paste0(out_path, 'Sechura_path_out1.csv'), sep = ';', row.names = F)
...
pngfile <- paste0(out_path, 'Sechura_path_out_mean1.png')
zlim <- c(-65,0)
mymap <- get_map(location = c(lon = sum(xlimmap/2), lat = sum(ylimmap/2)), zoom = 8, maptype = 'satellite', color='bw')
map   <- ggmap(mymap)
map   <- map +
...
if(!is.null(pngfile)) ggsave(filename = pngfile, width = 9, height = 9) else map
write.table(x = day_mean, file = paste0(out_path, 'Sechura_path_out_mean1.csv'), sep = ';', row.names = F)
