#===============================================================================
# Name   : count_particles_by_trajectory_lobos
# Author : Jorge Flores
# Date   : 
# Version:
# Aim    : -------
...
for(i in winds){ # Loop para cada tipo de forzante de viento
  simu <- paste0(i,'_lobos')    # Para rutas Lobos --> Sechura
...
write.table(x = daily_paths, file = paste0(out_path, 'daily_lobos_index_paths.csv'), row.names = F, sep = ',')


# clim_paths <- as.data.frame(List_wind[[2]])
# write.table(x = clim_paths, file = paste0(out_path, 'clim_lobos_index_paths.csv'), row.names = F, sep = ',')
