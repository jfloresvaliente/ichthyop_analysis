#===============================================================================
# Name   : count_particles_by_trajectory_lobos_sechura
# Author : Jorge Flores
# Date   : 
# Version:
# Aim    : -------
...
for(i in winds){ # Loop para cada tipo de forzante de viento
  simu <- paste0(i,'_lobos_sechura')    # Para rutas Lobos --> Sechura
  ...
  for(month in 1:12){ # Loop para cada mes
    ...
    # northward.in <- as.numeric(levels(factor(northward$Drifter)))
    # path1 <- rep(3, times = length(northward.in))
    # nor.month <- rep(month, times = length(northward.in))
    # path1 <- cbind(nor.month,northward.in, path1)
    
    # Indices de particulas (TODAS VAN AL NORTE)
    # Luego se evalua si entraron por el norte o sur de la isla
    # southward <- subset(df, df$Drifter %nin% northward.in)
    southward <- df
    southward.in <- as.numeric(levels(factor(southward$Drifter)))
    southward <- subset(df, df$Drifter %in% southward.in)
    
    # paths <- NULL
    path_particle_all <- NULL
    for(j in southward.in){ # Loop para evaluar cada particula restante que fue directo al sur
    ...
    # all_paths <- rbind(path_particle_all, path1)
    all_paths <- rbind(path_particle_all)
...
write.table(x = daily_paths, file = paste0(out_path, 'daily_lobos_sechura_index_paths.csv'), row.names = F, sep = ',')


# clim_paths <- as.data.frame(List_wind[[2]])
# write.table(x = clim_paths, file = paste0(out_path, 'clim_lobos_sechura_index_paths.csv'), row.names = F, sep = ',')
