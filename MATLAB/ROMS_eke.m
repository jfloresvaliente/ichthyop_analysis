...
directory = ['H:/Ascat_', simu, '/'];
ncload ([directory,'newperush_avg.Y2010.M12.newperush.nc'],...
    'mask_rho','lon_rho', 'lat_rho');
lon_rho = lon_rho-360;
mask_rho(mask_rho==0)=NaN;

XY = load(['C:/Users/ASUS/Desktop/contour_plots/eke2/' 'eke2.txt']);

% Get all the variables available in Zmean file.
ncload ([directory,'newperush_SmeanZ_', simu, '.nc'],'u','v','Z');
...
%% Choose Z level
zlev = [1]; % 9 = at 40 m depth % used 3 , 5 ,7 , 9, 11, 12, 13, 16

%% Loop for plot both regimen winds

eke_clim = zeros(12,1);
eke_daily= zeros(12,1);
...
%% eke serie temporal
mv2_mean = zeros(size(XY,1),1); % pre-allocate cell for each grid-point
for kk = 1:size(XY,1); % loop to get values only in grid-points selected
    mv2_mean(kk) = mv1(XY(kk,1),XY(kk,2));
end
mv2_mean = mean(mv2_mean);
eke_clim(ii) = mv2_mean;
% eke serie temporal
...
mv2_mean = zeros(size(XY,1),1); % pre-allocate cell for each grid-point
for kk = 1:size(XY,1); % loop to get values only in grid-points selected
    mv2_mean(kk) = mv2(XY(kk,1),XY(kk,2));
end
mv2_mean = mean(mv2_mean);
eke_daily(ii) = mv2_mean;
% eke serie temporal
...
file.name = ['C:/Users/ASUS/Desktop/contour_plots/eke2/' 'intensity_currents_' num2str(time) '_depth_' prof '.tif'];
img = getframe(gcf);
