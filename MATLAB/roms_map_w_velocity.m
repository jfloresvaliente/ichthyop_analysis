...
nc = ncload ([directory,'newperush_SmeanZ_', simu, '.nc'],'u','v','Z','w');

%% Choose Z level
zlev = [3 5 7 9 11 12 13 16]; % 9 = at 40 m depth % used 3 , 5 ,7 , 9, 11, 12, 13

for jj = 1:length(zlev); 

zlevel = zlev(jj); 
nv = 5; %% velocity vectors subsampling
var.range = [-0.0001 0.0001];

%FigHandle = figure('Position', [75, 95, 900, 550]);
%figure ;
%ha = tight_subplot(3,4,[.02 .015],[.1 .01],[.03 .01]); % plot parameters

for ii = 1:12; % number of months
%figure ;
...
text(-80.78,-5.35,'Vertical velocity (m/s)','FontSize', 16, 'FontWeight', 'bold');
%     if(ii == 12)
%         pos = get(gca, 'position');
%         cbr = colorbar('h');
%         set(gca, 'position', [pos(1) pos(2) pos(3) pos(4)]);
...
file.name = [directory 'W_currents_' simu '_' num2str(time) '_depth_' prof '.tif'];
img = getframe(gcf);
